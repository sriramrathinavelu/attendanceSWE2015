...
urlpatterns = [
	...
	url(r'^coursetype/(?P<course_key>[a-z0-9]+-\d+)/$', GetCourseType.as_view()),
	...
	url(r'^attendance/(?P<email>[^@]+@[^@]+\.[^/]+)/(?P<course_key>[a-z0-9]+-\d+)/(?P<filename>\w+\.?\w*)/$', TheAttendance.as_view()),
	url(r'^report/(?P<email>[^@]+@[^@]+\.[^/]+)/(?P<course_key>[a-z0-9]+-\d+)/$', GenerateReport.as_view()),
]
